...
public class Help extends Command {
    ...
    public void execute(CommandSender sender, String[] args) {
        if(args.length == 1) {
            ...
            sender.sendMessage(new ComponentBuilder(" * ").color(ChatColor.RED).append(new ComponentBuilder("[GODLIKE]").color(ChatColor.LIGHT_PURPLE).create()).append(new ComponentBuilder(" rank commands").color(ChatColor.AQUA).event(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/help godlike")).event(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text("§bView commands"))).create()).create());
            sender.sendMessage(new ComponentBuilder(" * ").color(ChatColor.RED).append(new ComponentBuilder("[LEGEND]").color(ChatColor.RED).create()).append(new ComponentBuilder(" rank commands").color(ChatColor.AQUA).event(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/help legend")).event(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text("§bView commands"))).create()).create());
